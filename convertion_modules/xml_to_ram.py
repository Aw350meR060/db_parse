...
def xml_to_ram(xml):
    schema = Schema()
    for attributeName, attributeValue in xml.documentElement.attributes.items():
        ...
        schema.domains = _getDomains(xml)
    ...
    for table in xml.getElementsByTagName("table"):
        ...
        for attributeName, attributeValue in table.attributes.items():
            if attributeName.lower() == "name":
                ...
                raise ValueError("Incorrect attribute name \"{}\" in tag \"{}\" ".format(attributeName, table.nodeName))
    ...
    for field in xml.getElementsByTagName("field"):
        ...
        for attributeName, attributeValue in field.attributes.items():
            if attributeName.lower() == "name":
                ...
                raise ValueError("Incorrect attribute name \"{}\" in tag \"{}\" ".format(attributeName, field.nodeName))

        fields.append(tmp)
    return fields

def _getConstraints(xml):
    if xml.nodeName != "table":
        raise ValueError("Is not a table")
    ...
    for index in xml.getElementsByTagName("index"):
        ...
        for attributeName, attributeValue in index.attributes.items():
            if attributeName.lower() == "field":
                ...
                raise ValueError("Incorrect attribute name \"{}\" in tag \"{}\" ".format(attributeName, index.nodeName))
        indexes.append(tmp)
    return indexes
